...
public class MenuFileService
{
			...
			List<Supports> Sup, Loading loading, List<Material> UserDefinedMat, List<Section> UserDefinedSec)
	{
		...
		SaveStructureToTxt(FileName, InputSections, InputVariables, values);		// Save the structure in an input file (.txt)
	}
	...
	public static void saveStructure(String filename, StructureDTO structureDTO)
	{
        ...
        {
            Gson gson = new GsonBuilder().setPrettyPrinting().excludeFieldsWithModifiers(java.lang.reflect.Modifier.STATIC).create();
	...
	private static void SaveStructureToTxt(String filename, String[] sections, String[][] vars, Object[][][] values)
	{
		try
		{	
