...
{	
	...
	public static void SaveFile(String FileName, String[][] AllText, String Language, MyCanvas MainCanvas, Structure Struct, Nodes[] Node, Elements[] Elem, Supports[] Sup, ConcLoads[] ConcLoads, DistLoads[] DistLoads, NodalDisps[] NodalDisps, double[][] UserDefinedMat, double[][] UserDefinedSec)
	{
		...
		{
			values[5] = new Object[UserDefinedSec.length][2];
			for (int sec = 0; sec <= UserDefinedSec.length - 1; sec += 1)
			{
				values[5][sec][0] = sec;
				values[5][sec][1] = UserDefinedSec[sec][0];
			}
		}
		if (Sup != null)
		{
		...
		{			
			...
			if (Input != null)
			{
				...
				for (int sec = 0; sec <= Input[5].length - 4; sec += 1)
				{
					...
					SecType[sec] = new double[] {Double.parseDouble(Line[1])};
				}
	...
	public static void AddSectionsToElements(int[] Elems, double[] Sec)
	{
	...
	public static void MouseWheel(int[] MainPanelPos, MyCanvas MainCanvas, int WheelRot, boolean[] AssignmentIsOn)
	{
		...
		if (AssignmentIsOn[1] & !MouseIsInMainCanvas)
		{
			...
			SelectedSec = Math.min(Math.max(SelectedSec, 0), SecType.length - 1);
		}
